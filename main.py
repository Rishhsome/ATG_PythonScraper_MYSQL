...
if __name__ == '__main__':
    ...
    for item in links_list1:
        # make a variable to store information regarding your SQL database (eg, database's name, password, username, etc)
        db = mysql.connector.connect(database="localhost", user="rishabh", passwd = "root")
        cursor = db.cursor()

        # Run a MySQL command to insert elements into a table
        add_AppData = ("INSERT INTO playstoredata"
                   "(AppName, MakerName, DownloadCount, Ratings, ReviewCount, Email, SimilarAppLinks)"
                   "VALUES (%s, %s, %s, %s, %s, %s, %s)")
        
        # Make a tuple of items of dictionary d1, to be inserted one by one into the database
        AppData = (d1["App Name"][i],d1["Maker's Name"][i],d1["Download Count"][i],d1["Rating (OUT OF 5)"][i],d1["Review Count"][i],d1["Email from Description"][i],d1["Similar App Links"][i])

        # Insert scraped data into database
        cursor.execute(add_AppData, AppData)
        db.commit()
